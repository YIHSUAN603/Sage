...
export default {
  ui: {
    ...
    settings: {
      ...
      wanderEnable: "让伙伴在桌面上自由走动（默认关闭）",
      wanderHint:
        "伙伴会自己决定往哪走（不需开启观察——没开时靠它的个性判断，开了观察再参考屏幕内容），并会不时随机散步。你拖动它或它正在说话时会暂停。系统不允许移动窗口的环境（部分 Linux）无效。",
      wanderHintRandom:
        "伙伴会在桌面上随机散步。开启主动搭话后，它就会自己决定往哪走（观察可开可不开）。你拖动它或它正在说话时会暂停。系统不允许移动窗口的环境（部分 Linux）无效。",
  ...
  prompt: {
    ...
    gate: {
      ...
      moveInstruction:
        "你也可以在桌面上走动。请在回复的最后「另起一行」，输出 `MOVE:` 加上下列其中一个：left、right、wander、corner、center、stay，代表你想往哪里去（例如靠过去一点、飘到角落不挡路、到处逛逛，或待在原地）。不想动就用 `stay`。那一行不要有其他文字，也不要在说出口的话里提到移动这件事。",
    },
    snapshot: {
      focused: "焦点组件：{{detail}}",
      selection: "选中文字：{{text}}",
    ...
    agent: {
      ...
      action_testing: "跑测试",
      action_reading: "读文件",
      action_searching: "搜索代码",
      action_executing: "执行命令",
      recent: "最近的往来片段（旧到新）：",
    },
    memory: {
      index: {
